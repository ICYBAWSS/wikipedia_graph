...
def get_next_output_path(base_name="massive_galaxy_static"):
    i = 1
    while True:
...
def render_massive_galaxy():
    ...
    output_image = get_next_output_path("massive_galaxy_static")
    ...
    # We do this first to determine the exact number of positions we actually have.
    ...
    # Distribute the orphan cluster at (0, 0)
    orphan_mask = (coords[:, 0] == 0.0) & (coords[:, 1] == 0.0)
    num_orphans = np.sum(orphan_mask)
    if num_orphans > 0:
        print(f"  Detected {num_orphans:,} orphans at (0, 0). Distributing them...")
        non_orphan_coords = coords[~orphan_mask]
        min_x, max_x = non_orphan_coords[:, 0].min(), non_orphan_coords[:, 0].max()
        min_y, max_y = non_orphan_coords[:, 1].min(), non_orphan_coords[:, 1].max()
        
        # Scatter them uniformly in the range of non-orphan nodes
        np.random.seed(42)
        coords[orphan_mask, 0] = np.random.uniform(min_x, max_x, num_orphans)
        coords[orphan_mask, 1] = np.random.uniform(min_y, max_y, num_orphans)

    num_coords = coords.shape[0]
    print(f"  Successfully loaded coordinates matrix of shape: {coords.shape}")

    # --- 2. INITIALIZE GRAPH STRUCTURE ---
    # Explicitly bound the graph to the maximum available coordinates to avoid segmentation faults
    ...
    # Fix: Re-introducing the 'pos' property mapping block
    pos = g.new_vertex_property("vector<double>")
    pos.set_2d_array(coords.T)
    
    # Fix: Generating and mapping the colors array to graph properties
